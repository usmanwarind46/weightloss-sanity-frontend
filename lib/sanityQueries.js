export const PAGE_QUERY = `
  *[_type == "page" && slug.current == $slug][0]{
    _id,
    title,
    slug,
    

    seo {
      metaTitle,
      metaDescription,
      canonical,
      noIndex,
      noFollow,

      ogTitle,
      ogDescription,
      ogImage{
        asset->{ url }
      },

      twitterTitle,
      twitterDescription,
      twitterImage{
        asset->{ url }
      },
      schema
    },

    sections[]{
      ...,

      _type == "mounjaroTabsSection" => {
        ...,

        tabs[]{
          label,
          id,
          htmlContent
        }
      },

      _type == "wegovyTabsSection" => {
        ...,

        tabs[]{
          label,
          id,
          htmlContent
        }
      },

      _type == "weightLossHero" => {
    ...,

        products[]{
          name,
          subtitle,
          price,
          bgColor,
          href,
          imageAlt,

          "image": image.asset->url, // ✅ NOW IT WILL WORK

          features[]{
            text,
            bold,
            suffix
          }
        }
      },  

      _type == "weightLossSteps" => {
        ...,
        steps[]{
          headingHTML,
          paragraph,

          description,
          bgColor,
          imageAlt,
          "image": image.asset->url
        }
      },

      _type == "weightLossTabs" => {
         ...,
      },

      _type == "weightLossMyths" => {
      ...,
      },

      _type == "weightLossComparison" => {
  ...,
  tableRows[]{
    ...,
    wegovyText[]{
      ...,
      markDefs[]{
        ...,
        _type == "link" => {
          href,
          blank
        }
      }
    },
    wegovyPillText[]{
      ...,
      markDefs[]{
        ...,
        _type == "link" => {
          href,
          blank
        }
      }
    },
    mounjaroText[]{
      ...,
      markDefs[]{
        ...,
        _type == "link" => {
          href,
          blank
        }
      }
    },
    foundayoText[]{
      ...,
      markDefs[]{
        ...,
        _type == "link" => {
          href,
          blank
        }
      }
    }
  }
},

      termsPageTitle,
      termsPageContentHTML,
      
      privacyPageTitle,
      privacyPageContentHTML,

      complaintsPageTitle,
      complaintsPageContentHTML,
      
      refundsPageTitle,
      refundsPageContentHTML,

      shippingPageTitle,
      shippingPageContentHTML,

      priceMatchGuaranteeTitle,
      priceMatchGuaranteeContentHTML,

      contactDetailsHeading,

      contactDetailsItems[]{
        type,
        value
      },

      pharmacyHeading,
      pharmacyAddressLines,

      clinicResourcesHeading,

      aboutResourcesItems[]{
        title,
        description,
        href,
        iconKey
      },

      aboutTabsItems[]{
        label,
        title,
        aboutTabsFooterNote,
        aboutTabsParagraphs,

        aboutTabsCards[]{
          text,
          theme,
          iconKey
        },

        aboutTabsFooterNote
      },

      introText,

      aboutCards[]{
        number,
        text,
        theme
      },

      footerNote,
      
      topHeading,
      topParagraph1,
      topParagraph2,

      leftHeading,
      leftParagraph1,
      leftParagraph2,

      rightHeading,

      trustItems[]{
        text,
        iconKey,
        theme
      },

      heading,
      paragraph1,
      paragraph2,
      leadText,

      bulletItems[]{
        text
      },

      desktopImage{
        asset->{ _id, url }
      },
      mobileImage{
        asset->{ _id, url }
      },

      imageAlt,

      badges[]{
        "imageUrl": image.asset->url,
        alt
      },

      steps[]{
        number,
        description,
        bgColor,
        imageAlt,
        "imageUrl": image.asset->url
      },

      mainImage{
        asset->{ url }
      },

      mainImageAlt,

      cards[]{
        title,
        description,
        bgColor,
        iconAlt,
        fullWidth,
        "iconUrl": icon.asset->url
      },

      _type == "statsSection" => {
        ...,
        statsCards[]{
          title,
          description,
          icon,
          iconAlt,
          iconColor
        }
      },

      reviews[]{
        name,
        role,
        review,
        rating,
        imageAlt,
        "imageUrl": image.asset->url
      },

      expectationItems[]{
        title,
        description,
        iconKey
      },

      _type == "aboutClinicExpectations" => {
          ...,
          heading,
          introText,

          sectionImage{
            asset->{ url }
          },

          imageAlt,

          expectationItems[]{
            title,
            description,
            iconKey
          }
        },


      categories[]{
      category,
      faqs[]{
        question,
        answer
        }
      },

      aboutRegulatorItems[]{
        shortName,
        fullName,
        description,
        logo{
          asset->{ url }
        },
        imageAlt,
        imageWidth,
        imageHeight,
      },

      _type == "wegovyHero" => {
        ...,

        productImages[]{
          imageAlt,
          "imageUrl": image.asset->url
        },

        featureItems[]{
          text
        },

        dosages[]{
          label,
          price
        },

        doctorImage{
          asset->{ url }
        },

        "trustpilotImageUrl": trustpilotImage.asset->url,
        trustpilotImageAlt,
        trustpilotLink
      },

      _type == "wegovyJourney" => {
        ...,

        sectionImage{
          asset->{ url }
        },

        journeyItems[]{
          title,
          content
        }
      },

      _type == "wegovyWeightLoss" => {
        ...
      },

      _type == "wegovyCTA" => {
        ...,

        image{
          asset->{ url }
        }
      },

      _type == "wegovyFaq" => {
        ...,

        faqs[]{
          question,
          answer
        }
      },

      _type == "wegovyBottomCTA" => {
        ...
      },

      _type == "mounjaroHero" => {
        ...,

        productImages[]{
          imageAlt,
          "imageUrl": image.asset->url
        },

        featureItems[]{
          text
        },

        dosages[]{
          label,
          price
        },

        doctorImage{
          asset->{ url }
        },

        "trustpilotImageUrl": trustpilotImage.asset->url,
        trustpilotImageAlt,
        trustpilotLink
      },

      _type == "mounjaroJourney" => {
          ...,
          paragraph[]{
            ...,
            markDefs[]{
              ...,
              _type == "link" => {
                href,
                blank
              }
            }
          },
          sectionImage{ asset->{ url } },
          journeyItems[]{ title, content }
        },

      _type == "mounjaroWeightLoss" => {
        ...
      },

      _type == "mounjaroCTA" => {
        ...,

        image{
          asset->{ url }
        }
      },

      _type == "mounjaroFaq" => {
        ...,

        faqs[]{
          question,
          answerHTML
        }
      },

      
    }
  }
`;

export const SITE_SETTINGS_QUERY = `
*[_type == "siteSettings"][0]{
  logo{
    asset->{ url }
  },
  topBarText,
  topBarRightText,

  navLinks[]{
    label,
    href
  },

  ctaLabel,
  ctaLink,

  footerLogo{
    asset->{ url }
  },

  footerDescription,
  footerLinks1,
  footerLinks2,
  footerContact{
    phone,
    email,
    address,
  },
  pharmacy{
    name,
    registrationNumber,
    registrationLink,
    prescriberName,
    prescriberGmc,
    prescriberLink,
    description,
  },
  footerBottomText,
}
`;

export const SEO_QUERY = `
*[_type == "seo"][0]{
  defaultTitle,
  titleTemplate,
  defaultDescription,
  siteUrl,
  allowAll,
  disallowPaths,
  sitemapUrl,
  sitemapCustomRoutes,
  


  defaultNoIndex,
  defaultNoFollow,

  ogSiteName,
  ogType,

  defaultOgImage{
    asset->{ url }
  },

  twitterCard,

  robotsContent,
  additionalMetaTags
}
`;

export const BLOGS_QUERY = `
*[_type == "post" && !(_id in path("drafts.**"))] | order(publishedAt desc){
  _id,
  title,
  slug,

  "image": featuredImage.asset->url,
  "imageAlt": featuredImage.alt,

  author->{
    name,
    slug,
    hasProfilePage
  },

  expert->{
    name,
    slug,
    hasProfilePage
  },

  categories[]->{
    title,
    slug
  }
}
`;

export const SINGLE_BLOG_QUERY = `
*[_type == "post" && slug.current == $slug][0]{
  _id,
  title,
  slug,
  publishedAt,
  _type,

  "image": featuredImage.asset->url,

  content[]{
    ...,

    _type == "image" => {
      ...,
      asset->{
        url
      }
    },

    _type == "blogSingleProductCard" => {
      ...,

      "imageUrl": image.asset->url,

      imageAlt
    },

    _type == "blogTable" => {
        ...,
        headers[],
        rows[]{
          cells[]
        }
      },
   _type == "blogTableRich" => {
      ...,
      headers[],
      rows[]{
        cells[]{
          content[]{
            ...,
            markDefs[]{
              ...,
              _type == "link" => {
                href,
                blank
              }
            }
          }
        }
      }
    }
  },

  

    "relatedPosts": *[
    _type == "post" &&
    slug.current != $slug &&
    categories[0]._ref == ^.categories[0]._ref
  ][0...10]{
    _id,
    title,
    slug,
    excerpt,
    "image": featuredImage.asset->url,
    "imageAlt": featuredImage.alt,
    categories[]->{
      title,
      slug
    }
  },

  author->{
    name,
    slug,
    hasProfilePage
  },

  expert->{
    name,
    slug,
    hasProfilePage
  },

  categories[]->{
    title,
    slug
  },

  seo
}
`;

export const CATEGORIES_QUERY = `*[_type == "category"] | order(title asc){ _id, title, slug }`;

export const AUTHOR_QUERY = `*[_type == "author" && slug.current == $slug][0]{ name, slug, role, bio, about, expertise, medicalExpertise, gmcNumber, gmcLink, specialisms, qualifications, "image": image.asset->url }`;

export const EXPERT_QUERY = `*[_type == "expert" && slug.current == $slug][0]{ name, slug, role, bio, about, "expertise": credentials, medicalExpertise, gmcNumber, gmcLink, specialisms, qualifications, "image": image.asset->url }`;

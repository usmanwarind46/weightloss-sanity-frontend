export function generateSchema({ data, globalSeo, canonical, manualSchema }) {
  const schemas = [];
  ...
  // 🟢 4. BREADCRUMB
  // ─────────────────────────────
  const hasManualBreadcrumb =
    manualSchema && manualSchema.includes('"BreadcrumbList"');
  if (canonical && !hasManualBreadcrumb) {

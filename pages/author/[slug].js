...
export async function getStaticProps({ params }) {
  const data = await sanityClient.fetch(AUTHOR_QUERY, {
    slug: params.slug,
  });
...
// PAGE
// ==============================
const AuthorDetails = ({ data, seoSettings, siteSettings }) => {
  const autoSchemas = generateSchema({
    globalSeo: seoSettings,
    canonical: `${meta_url}/author/${data?.slug?.current}`,
  });
        ...
        {data?.gmcNumber && (
          <div className="mt-8">
            ...
            {data.gmcLink ? (
              ...
              <p className="text-gray-700 text-auther">
                GMC reference no: {data.gmcNumber}
              </p>
            )}
          </div>
